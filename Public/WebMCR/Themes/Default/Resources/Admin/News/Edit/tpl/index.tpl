...
<html>
	<head>
		...
		<link href="{{__META__.theme_url}}Resources/Admin/News/Edit/css/style.css?1" rel="stylesheet">
		<link href="{{__META__.theme_url}}Resources/Admin/News/Edit/css/style-responsive.css?1" rel="stylesheet">
		<script src="{{__META__.theme_url}}Resources/Admin/News/Edit/js/script.js?1"></script>
	</head>
	...
	<body>
		...
		<div class="admin">
			...
			<div class="block-right">
				<form action="{{ __META__.site_url }}admin/news/edit/{{ item.id }}/submit" method="POST" enctype="multipart/form-data" class="news-edit">
						...
						<div class="form-wrapper">
							<div>
								<div class="window">
									...
									{% if item.image %}
										<div class="image-preview" style="background-image: url('{{ item.image }}')" id="preview"></div>
                                    {% else %}
									...
									<div class="input-block">
										...
										<div class="input-append">
											...
											<input type="text" id="image" value="{{ item.image }}" name="image" placeholder="Введите URL изображения">
							...
							<div>
								<div class="window">
									<div class="input-block">
										...
										<textarea name="text_short" id="text_short" rows="5" placeholder="Введите краткое описание">{{ item.text_short_bb }}</textarea>
									</div>

									<div class="input-block m-0">
										<label for="text" class="text-bold">Полное описание</label>
										<textarea name="text" class="m-0" id="text" rows="16" placeholder="Введите полное описание">{{ item.text_bb }}</textarea>
									</div>
								</div>
							</div>
						</div>
					...
					<div class="form-bottom window w-50 mx-auto">
						{% if tags %}
							...
							<div class="tags" data-input-target="#tags-input">
								<ul>
                                    {% for tag in tags %}
										{% if tag.id|int in selected_tags %}
											{% set selected = true %}
										{% else %}
											{% set selected = false %}
										{% endif %}
										<li class="{% if selected %}active{% endif %}">
											<label title="{{ tag.text }}"><input type="checkbox" class="m-0 tag-selector" {% if selected %}checked{% endif %} name="tags[]" value="{{ tag.id }}"> {{ tag.title }}</label>
										</li>
                                    {% endfor %}
								</ul>
							</div>
