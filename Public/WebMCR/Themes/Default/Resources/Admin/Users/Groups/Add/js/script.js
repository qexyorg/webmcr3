$(function(){
	$('body').on('click', '.groups-add [type="submit"]', function(e){
		e.preventDefault();
		...
		qx.load_elements(form.attr('action'), qx.getFormValues(form), function(data){
			...
			form.find('.form-wrapper').fadeOut('fast', function(){
				...
				form.find('.form-success').fadeIn('fast', function(){
					setTimeout(function(){
						window.location.href = meta.site_url+'admin/groups/';
					}, 1200);
				});
			});
	...
	}).on('input', '.groups-add #title', function(){
		$('.groups-add #name').val(qx.translate($(this).val()));
	});
});